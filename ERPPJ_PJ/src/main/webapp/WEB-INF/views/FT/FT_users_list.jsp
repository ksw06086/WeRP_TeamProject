...
<script type="text/javascript">
    	...
    	$(function(){
    		...
    		$.ajax({
                type : "GET",
                url : "/erp/FT_UsersSelect?${_csrf.parameterName }=${_csrf.token }&srhval=${key}",
                success : function(data) {
           	   if(data != null){
	                   $("tbody").html("");
						for(i = 0; i < data.length; i++){
							$("tbody").append('<tr>' +
              				'<td id = "code' + i + '" onclick="focuse(' + i + ');" style = "width: 80px;" ondblclick="setvalue();">' + data[i].username + '</td>' +
              				'<td id = "name' + i + '" onclick="focuse(' + i + ');" ondblclick="setvalue();" style = "width: calc( 200px - 16px );">' + data[i].e_name + '</td>' +
              			'</tr>');
						}
           	   }
                },
	    ...
	    function search(val){
    		...
    		var jsonData;
       		$.ajax({
                      type : "GET",
                      url : "/erp/FT_UsersSelect?${_csrf.parameterName }=${_csrf.token }&srhval=" + val,
                      success : function(data) {
	             	   if(data != null){
		                   $("tbody").html("");
							for(i = 0; i < data.length; i++){
											...
											<c:forEach var="sub" items="${users}">
												<tr>
													<td id="code${count}" onclick="focuse(${count});"
														style="width: 80px;" ondblclick="setvalue();">${sub.username}</td>
													<td id="name${count}" onclick="focuse(${count});"
														ondblclick="setvalue();"
														style="width: calc(200px - 16px);">${sub.e_name}</td>
