...
<script type="text/javascript">
    	...
    	$(function(){
    		...
    		$.ajax({
                type : "GET",
                url : "/erp/FT_DepartmentSelect?${_csrf.parameterName }=${_csrf.token }&srhval=${key}",
                success : function(data) {
           	   if(data != null){
	                   $("tbody").html("");
						for(i = 0; i < data.length; i++){
							$("tbody").append('<tr>' +
              				'<td id = "code' + i + '" onclick="focuse(' + i + ');" style = "width: 80px;" ondblclick="setvalue();">' + data[i].department_code + '</td>' +
              				'<td id = "name' + i + '" onclick="focuse(' + i + ');" ondblclick="setvalue();" style = "width: calc( 200px - 16px );">' + data[i].department_name + '</td>' +
              			'</tr>');
						}
           	   }
                },
	    ...
	    function setvalue(val){
			$("input[name=departmentCode${keyname}]", opener.document).val(scode);
    		$("input[name=departmentName${keyname}]", opener.document).val(sname);
	    ...
	    function search(val){
    		...
    		var jsonData;
       		$.ajax({
                      type : "GET",
                      url : "/erp/FT_DepartmentSelect?${_csrf.parameterName }=${_csrf.token }&srhval=" + val,
                      success : function(data) {
	             	   if(data != null){
		                   $("tbody").html("");
							for(i = 0; i < data.length; i++){
						...
						<div class="card">
							<div class="card-body" style="padding-bottom: 0px;">
								<div style="text-align: center;">
									<label>Search : <input type="search" id="searchid"
										class="form-control form-control-sm" aria-controls="datatable"
										style="display: inline-block; width: 150px;"
										onkeyup="search(this.value);"></label>
								</div>
								<div align="right">
								...
								<table id="datatable"
									...
									<tbody style="overflow: auto;">
										...
										<c:if test="${department != null}">
											<c:forEach var="sub" items="${department}">
												<tr>
													<td id="code${count}" onclick="focuse(${count});"
														style="width: 80px;" ondblclick="setvalue();">${sub.department_code}</td>
													<td id="name${count}" onclick="focuse(${count});"
														ondblclick="setvalue();"
														style="width: calc(200px - 16px);">${sub.department_name}</td>
